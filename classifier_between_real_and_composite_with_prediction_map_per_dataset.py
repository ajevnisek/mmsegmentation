...
if args.is_cluster == True:
    ...
    IMAGES_DATASET_ROOT = os.path.join('..', 'data',
                                       'Image_Harmonization_Dataset', DATASET,)
    MASKS_DATASET_ROOT = os.path.join('../data/MaskPredictionDataset/', DATASET)
    TARGET_ROOT = os.path.join(
        'CompositeAndRealImagesClassifierWithPredictionMaps', DATASET,
        datetime.now().strftime("%Y_%m_%d__%H_%M_%S"))
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'test']:
            ...
            for samples in dataloaders[phase]:
                ...
                image = samples['image']
                mask = samples['mask']
                inputs = torch.cat([image, mask], axis=1)
...
class RealFakeDataset(torch.utils.data.Dataset):
    def __init__(self, real_images, fake_images, real_images_masks,
                 fake_images_masks, images_transform=None, masks_transform=None):
        super().__init__()
        self.real_images = real_images
        self.fake_images = fake_images
        self.real_images_masks = real_images_masks
        self.fake_images_masks = fake_images_masks
        self.images_transform = images_transform
        self.masks_transform = masks_transform

        self.all_images = self.real_images + self.fake_images
        self.all_masks = self.real_images_masks + self.fake_images_masks
        self.num_real_images = len(self.real_images)

    def __getitem__(self, item):
        image = Image.open(self.all_images[item])
        mask = Image.open(self.all_masks[item])
...
for split in ['train', 'test']:
    ...
    fake_images[split] = [os.path.join(IMAGES_DATASET_ROOT,
                                       'composite_images', image_name)
                          for image_name in fake_images_names]
    real_images[split] = [os.path.join(
        IMAGES_DATASET_ROOT, 'real_images',
        convert_composite_image_name_to_real_image_name(image_name))
                          for image_name in fake_images_names]
    fake_images_maps[split] = [os.path.join(
        MASKS_DATASET_ROOT, 'composite_images',image_name)
        for image_name in fake_images_names]
    real_images_maps[split] = [os.path.join(
        MASKS_DATASET_ROOT, 'real_images',
        convert_composite_image_name_to_real_image_name(image_name))
                          for image_name in fake_images_names]


image_datasets = {x: RealFakeDataset(real_images[x], fake_images[x],
                                     real_images_maps[x], fake_images_maps[x],
                                     images_transforms[x], masks_transforms[x])
                  for x in ['train', 'test']}
