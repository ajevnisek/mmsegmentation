...
if args.is_cluster:
    ...
    target_root = '/storage/jevnisek/MaskPredictionResults' \
                  '/mask_prediction_on_real_images_results/'
else:
    data_root = os.path.join(f'../data/Image_Harmonization_Dataset/',
                             IMAGE_HARMONIZATION_DATASET)
    target_root = os.path.join('mask_prediction_on_real_images_results',)
...
for test_image_name in tqdm(images_to_scan):
    ...
    overlaid_ground_truth = model.show_result(
        img, ToTensor()(Image.open(mask)), palette=GT_PALETTE, show=False,
                                              opacity=0.5)
    ...
    +++++++++++++++++++++++++++++++++++++++++++++++
    + real          | composite                   +
    + ground truth  | estimated map on real image +
    +++++++++++++++++++++++++++++++++++++++++++++++
    """
    ...
    concatenated_image.resize((1024, 1024)).save(os.path.join(
        target_root,
        IMAGE_HARMONIZATION_DATASET,
        f"{test_image_name[:-4]}_on_real_image.jpg"))
