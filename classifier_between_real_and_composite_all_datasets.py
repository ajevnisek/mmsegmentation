...
if args.is_cluster == True:
    ...
    TARGET_ROOT = os.path.join(
        '/storage/jevnisek/CompositeAndRealImagesClassifierAllDatasets',
        datetime.now().strftime("%Y_%m_%d__%H_%M_%S"))
else:
    DATASET_ROOT = os.path.join('..', 'data', 'Image_Harmonization_Dataset',)
    TARGET_ROOT = os.path.join(
        'CompositeAndRealImagesClassifierAllDatasets',
        datetime.now().strftime("%Y_%m_%d__%H_%M_%S"))
...
for split in ['train', 'test']:
    for dataset in DATASETS:
        split_path = os.path.join(DATASET_ROOT, dataset, f'{dataset}_{split}.txt')
        fake_images_names = mmcv.list_from_file(split_path)
        fake_images.setdefault(split, [])
        fake_images[split].append([
            os.path.join(DATASET_ROOT, dataset, 'composite_images', image_name)
            for image_name in fake_images_names])
        real_images.setdefault(split, [])
        real_images[split].append([
            os.path.join(DATASET_ROOT, dataset, 'real_images',
                         convert_composite_image_name_to_real_image_name(
                             image_name))
            for image_name in fake_images_names])

for split in ['train', 'test']:
    fake_images[split] = sum(fake_images[split], [])
    real_images[split] = sum(real_images[split], [])
